...
bounds = [(0.01, 1.0),(0.5,0.999),(1.0,5.0),(5.0,15.0),(1.0,10.0)]  # Bounds [(x1_min, x1_max), (x2_min, x2_max),...]
...
def plot_graficos(t_range, solve, t_exp, data_exp,path,i):
    ...
    plt.title(f'Carga viral no tempo Paciente 4 (execução {str(i+1)})')
    plt.ylabel("Carga viral ($log_{10}$ UI/mL)")
    plt.xlabel("Tempo (dias)")
    plt.yscale('log')
    plt.legend()
    #plt.show()
    plt.savefig(f'{path}/PAT4_{str(i+1)}.png',dpi=300)
    #plt.clf()
