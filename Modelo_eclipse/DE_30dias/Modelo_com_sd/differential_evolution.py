...
bounds = [(0.01, 1.0),(0.9995,0.9999),(3.0,5.0),(5.0,10.0),(2.0,7.0)]  # Bounds [(x1_min, x1_max), (x2_min, x2_max),...]
...
t_avg = [0.04, 0.08, 0.17, 0.34, 0.50, 1.00, 1.50, 2.96, 3.94, 7.94, 9.95, 14.94, 24.03, 30.94]
# Media do resultado experimental
avg = [6.47991433, 6.42897983, 5.857277, 4.63766183, 4.08108333, 3.38275467,
...
def plot_graficos(t_range, solve, t_exp, data_exp,path,i):
    ...
    plt.title(f'Carga viral no tempo Average (execução {str(i+1)})')
    plt.ylabel("Carga viral ($log_{10}$ UI/mL)")
    plt.xlabel("Tempo (dias)")
    plt.yscale('log')
    plt.legend()
    #plt.show()
    plt.savefig(f'{path}/Average{str(i+1)}.png',dpi=300)
    #plt.clf()
...
if __name__ == "__main__":
    ...
    pwd = os.getcwd()
    path = pwd
    saida = open(f'{path}/relatorio.txt', "a+")
    ...
    while j<10:
        ...
        saida.writelines('Execution ' + str(j +1))
        ...
        plot_graficos(t_range,sol,t_exp_total[i],patients[i],path,j)
        final = time.time()
        saida.writelines('\nTempo de execucao: ' + str(final - inicio))
        j+=1
