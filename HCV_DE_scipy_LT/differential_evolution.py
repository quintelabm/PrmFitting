...
popsize = 2                                              # Population size, must be >= 4
...
# TODO continuar para os outros pacientes antes de colocar para rodar
...
if __name__ == "__main__":
    ...
    cwd = os.getcwd()
    print(cwd)
    saida = open(cwd+"/HCV_DE_scipy_LT/outputs/" + str(PAT) + ".txt", "a+")
    saida.writelines("\n\n-------NOVA TENTATIVA-------\n\n")
    saida.writelines('Population size: '+ str(popsize)+ '\nNumber of generations: '+ str(maxiter)+ '\n')
    ...
    plt.savefig(cwd+"/HCV_DE_scipy_LT/outputs/"+PAT+".png")
